...
// Los obtienes en: Firebase Console → Configuración del proyecto → Tus apps → Web
...
const App = () => {
  ...
  React.useEffect(() => {
    console.log("App iniciada v2.0 - con botón Agregar Sector");
  }, []);
  ...
  // Guardar en Firebase cuando cambian las zonas (con debounce)
  ...
  // Funciones para editar layout
  const handleAddZone = () => {
    const newZone = {
      id: `z-${Date.now()}`,
      name: 'Nueva Zona',
      type: ZoneType.GONDOLA,
      x: 5, y: 5, w: 2, h: 2,
      productIds: [],
      capacity: 4,
  ...
  const handleDeleteZone = (zoneId) => {
    if (confirm('¿Eliminar esta zona? Los productos volverán al inventario.')) {
  ...
  return React.createElement('div', { className: 'flex h-screen w-full bg-gray-50 text-slate-800' },
      ...
      React.createElement('header', { className: 'h-16 bg-white border-b border-gray-200 px-6 flex items-center justify-between shadow-sm z-10 shrink-0' },
        ...
        React.createElement('div', { className: 'flex items-center gap-3' },
          ...
          isLayoutMode ? React.createElement('button', {
            onClick: () => {
              console.log("Click en Agregar Sector");
              handleAddZone();
            },
            className: 'bg-orange-600 hover:bg-orange-700 text-white px-3 py-1.5 rounded-md text-sm font-medium flex items-center gap-2 shadow-sm animate-pulse'
          }, '➕ AGREGAR SECTOR') : null,
    ...
    // Layout Editor Modal (cuando hay zona en edición en modo layout)
    isLayoutMode && editingZone && React.createElement('div', { className: 'fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4' },
      React.createElement('div', { className: 'bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden' },
        React.createElement('div', { className: 'px-6 py-4 border-b flex justify-between items-center bg-orange-50' },
          React.createElement('h3', { className: 'text-lg font-bold text-orange-900' }, '⚙️ Editar Zona'),
          React.createElement('button', { onClick: () => setEditingZoneId(null), className: 'p-1 hover:bg-orange-100 rounded text-orange-700' }, '✕')
        ),
        React.createElement('div', { className: 'p-6 space-y-4' },
          // Nombre
          React.createElement('div', {},
            React.createElement('label', { className: 'text-xs font-bold text-gray-500 uppercase mb-1 block' }, 'Nombre'),
            React.createElement('input', { type: 'text', value: editingZone.name, onChange: (e) => handleUpdateZoneConfig(editingZone.id, { name: e.target.value }), className: 'w-full border rounded-md p-2 text-sm' })
          ),
          // Tipo y Capacidad
          React.createElement('div', { className: 'grid grid-cols-2 gap-4' },
            React.createElement('div', {},
              React.createElement('label', { className: 'text-xs font-bold text-gray-500 uppercase mb-1 block' }, 'Tipo'),
              React.createElement('select', { value: editingZone.type, onChange: (e) => handleUpdateZoneConfig(editingZone.id, { type: e.target.value }), className: 'w-full border rounded-md p-2 text-sm' },
                Object.values(ZoneType).map(t => React.createElement('option', { key: t, value: t }, t))
              )
            ),
            React.createElement('div', {},
              React.createElement('label', { className: 'text-xs font-bold text-gray-500 uppercase mb-1 block' }, 'Capacidad'),
              React.createElement('input', { type: 'number', min: 1, value: editingZone.capacity, onChange: (e) => handleUpdateZoneConfig(editingZone.id, { capacity: parseInt(e.target.value) || 1 }), className: 'w-full border rounded-md p-2 text-sm' })
            )
          ),
          // Posición y Tamaño
          React.createElement('div', { className: 'bg-gray-50 p-4 rounded-lg border' },
            React.createElement('label', { className: 'text-xs font-bold text-gray-500 uppercase mb-2 block' }, '📐 Posición y Tamaño (Grilla 10x12)'),
            React.createElement('div', { className: 'grid grid-cols-4 gap-2' },
              React.createElement('div', {},
                React.createElement('span', { className: 'text-[10px] text-gray-400' }, 'Col (X)'),
                React.createElement('input', { type: 'number', min: 1, max: 10, value: editingZone.x, onChange: (e) => handleUpdateZoneConfig(editingZone.id, { x: Math.min(10, Math.max(1, parseInt(e.target.value) || 1)) }), className: 'w-full border rounded p-1.5 text-sm' })
              ),
              React.createElement('div', {},
                React.createElement('span', { className: 'text-[10px] text-gray-400' }, 'Fila (Y)'),
                React.createElement('input', { type: 'number', min: 1, max: 12, value: editingZone.y, onChange: (e) => handleUpdateZoneConfig(editingZone.id, { y: Math.min(12, Math.max(1, parseInt(e.target.value) || 1)) }), className: 'w-full border rounded p-1.5 text-sm' })
              ),
              React.createElement('div', {},
                React.createElement('span', { className: 'text-[10px] text-gray-400' }, 'Ancho'),
                React.createElement('input', { type: 'number', min: 1, max: 10, value: editingZone.w, onChange: (e) => handleUpdateZoneConfig(editingZone.id, { w: Math.min(10, Math.max(1, parseInt(e.target.value) || 1)) }), className: 'w-full border rounded p-1.5 text-sm' })
              ),
              React.createElement('div', {},
                React.createElement('span', { className: 'text-[10px] text-gray-400' }, 'Alto'),
                React.createElement('input', { type: 'number', min: 1, max: 12, value: editingZone.h, onChange: (e) => handleUpdateZoneConfig(editingZone.id, { h: Math.min(12, Math.max(1, parseInt(e.target.value) || 1)) }), className: 'w-full border rounded p-1.5 text-sm' })
              )
            )
          ),
          // Botones
          React.createElement('div', { className: 'flex justify-between pt-2' },
            React.createElement('button', { onClick: () => handleDeleteZone(editingZone.id), className: 'text-red-600 hover:bg-red-50 px-3 py-2 rounded-md text-sm font-medium' }, '🗑 Eliminar Zona'),
